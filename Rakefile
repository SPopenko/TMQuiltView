PROJECT_NAME = "TMQuiltView"
APP_NAME = "OCUnitApp"
CONFIGURATION = "Debug"

SPECS_TARGET_NAME = "Specs"
UI_SPECS_TARGET_NAME = "TMQuiltViewTests"
...
task :specs => :build_specs do
  build_dir = build_dir("")
  with_env_vars("DYLD_FRAMEWORK_PATH" => build_dir, "CEDAR_REPORTER_CLASS" => "CDRColorizedReporter,CDRJUnitXMLReporter") do
...
desc "Run UI specs simulator"
task :uispecsiPhone => :build_uispecs do
  env_vars = {
    "DYLD_ROOT_PATH" => sdk_dir,
    "IPHONE_SIMULATOR_ROOT" => sdk_dir,
    "CFFIXED_USER_HOME" => Dir.tmpdir,
  ...
  with_env_vars(env_vars) do
    system_or_exit "waxsim #{File.join(build_dir("-iphonesimulator"), "#{UI_SPECS_TARGET_NAME}.app")}";
  end
end
